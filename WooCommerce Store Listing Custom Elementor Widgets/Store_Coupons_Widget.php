	...
	class Dynamic_Store_Coupons_Widget extends \Elementor\Widget_Base {
		...
		protected function _register_controls() {
			...
			$this->add_control(
			                'percentage_color',
			                [
			                    'label' => __('Percentage Color', 'my-custom-theme'),
			                    'type' => \Elementor\Controls_Manager::COLOR,
			                    'selectors' => [
			                        '{{WRAPPER}} .coupon-percentage' => 'color: {{VALUE}}',
			                    ],
			                ]
			            );
			$this->add_group_control(
			                ...
			                [
			                    ...
			                    'label' => __('Percentage Typography', 'my-custom-theme'),
			                    'selector' => '{{WRAPPER}} .coupon-percentage',
			                ]
			            );
			$this->add_control(
			                'description_color',
		...
		private function render_retailmenot_coupon_item($coupon_id, $settings) {
			$percentage = get_field('percentage', $coupon_id);
			$coupon_title = get_the_title($coupon_id);
			$coupon_description = get_the_content(null, false, $coupon_id);
			$button_text = get_field('button_text', $coupon_id);
			$button_text = !empty($button_text) ? $button_text : __('Get Deal', 'my-custom-theme');
			$coupon_button_link = get_field('coupon_button_link', $coupon_id);
			$coupon_terms_button = get_field('coupon_terms_button', $coupon_id);
			$coupon_terms_accordion = get_field('coupon_terms_accordion', $coupon_id);
			?>
			            <div class="coupon-item">
			                <div class="coupon-content">
			                    <div class="coupon-left">
			                        <?php if (!empty($percentage)): ?>
			                            <div class="coupon-percentage"><?php echo esc_html($percentage);
			?></div>
			                        <?php endif;
			?>
			                    </div>
			                    <div class="coupon-center">
			                        <?php if (!empty($coupon_terms_button)): ?>
			                            <span class="coupon-selection-button"><?php echo esc_html($coupon_terms_button);
			?></span>
			                        <?php endif;
			?>
			                        <div class="coupon-title"><?php echo esc_html($coupon_title);
			?></div>
			                        <?php if (!empty($coupon_description)): ?>
			                            <div class="coupon-description"><?php echo wp_kses_post($coupon_description);
			?></div>
			                        <?php endif;
			?>
			                    </div>
			                    <div class="coupon-right">
			                        <?php if (!empty($button_text)): ?>
			                            <a href="<?php echo esc_url($coupon_button_link); ?>" class="coupon-button" target="_blank"><?php echo esc_html($button_text);
			?></a>
			                        <?php endif;
			?>
			                    </div>
			                ...
			                <?php if (!empty($coupon_terms_accordion)): ?>
			                    <div class="coupon-footer">
			                        <a href="#" class="see-details">
			                            <?php echo __('See Details', 'my-custom-theme');
		...
		private function render_styleless_coupon_item($coupon_id, $settings) {
			...
			?></div>
			                <?php endif;
			?>
			                <?php if (!empty($coupon_button_link)): ?>
			                    <a href="<?php echo esc_url($coupon_button_link); ?>" class="coupon-button" target="_blank"><?php echo esc_html($button_text);
			?></a>
			                <?php endif;
			?>
			                <?php if (!empty($coupon_terms_accordion)): ?>
			                    <a href="#" class="see-details"><?php echo __('See Details', 'my-custom-theme');
			?> <span class="details-icon">+</span></a>
			                    <div class="coupon-details" style="display:none;">
			                        <?php echo wp_kses_post($coupon_terms_accordion);
			?>
			                    </div>
